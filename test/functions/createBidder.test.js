...
async function callWrappedCreateBidder(pin, uid) {
  ...
  await test.wrap(createBidder)(snap, context)
}
...
describe("The function creating a new bidder", function () {
  describe("triggered by a PIN written to the index", function () {
    it("doesn't create the bidder if there is no auction", async function () {
      ...
      assert.equal(
        seat.exists(),
        false,
        `There was a seat in an nonexistent auction`
      )
    })
